...
files = list.files(pattern = ".*sf", 
                   path =  "./data/4_salmon_count_re_analysis/", 
                   recursive = T,
                   full.names = T)
# add names
names(files) = list.files(path =  "./data/4_salmon_count_re_analysis/")
...
maplot_pretty <- ggplot(forplots_3, aes(x = log2(basemean),
  ...
  labs(x = "log2(Mean of Normalized counts)", y = "log2(fold change)",
       title = expression(paste("Gene expression in N2 vs ", italic("fbxa-215"))),
       color = "Adj. p-value") +
  theme_bw(base_size = 14)

maplot_pretty + coord_cartesian(ylim = c(-13,13))
