...
files = list.files(pattern = ".*sf", 
                   path =  "./data/4_salmon_count/", 
                   recursive = T,
                   full.names = T)
# add names
names(files) = list.files(path =  "./data/4_salmon_count/")
...
##  Non - Replicate comparison plots
pdf("./analysis/figures/scatterPlotsNonReps.pdf", height = 15, width = 15)
done = c()
par(mfrow=c(2,2))
for (i in colnames(ddsObj)){
    for( j in colnames(ddsObj)){
...
#FDR is alpha on line 226. silencing lines 227-229 to set up a log fold change myself, to not shrink lfc.
...
maplot <- ggplot(forplots_3, aes(x = log2(basemean),
    ...
    geom_text_repel(
        data = subset(forplots_3, highlight == "yes"),
...
maplot + coord_cartesian(ylim = c(-15,15))

## 6.51  x 5.35 for paper
...
maplot2 + coord_cartesian(ylim = c(-13,13))
