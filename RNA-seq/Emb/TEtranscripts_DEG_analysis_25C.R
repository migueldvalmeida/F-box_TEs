...
# Choose the files 
TEcounts = read_table("./6_TEtranscripts/fbxa215_TEtranscripts_25oC.cntTable")
...
colnames(TEcounts)[2:9] <- str_replace(colnames(TEcounts)[2:9],
                                       '.+_STARTEtranscripts//(.+)', '\\1')
colnames(TEcounts)
...
sampleInfoTE = as.data.frame(colnames(TEcounts[2:9]))
sampleInfoTE$group = sub("_.*", "", sampleInfoTE$`colnames(TEcounts[2:9])`)
...
TEcounts_matrix <- TEcounts_matrix[,2:9]
head(TEcounts_matrix)
...
## write DEseq TE table for paper
...
write.table(TEs_DESeq2_normCounts, file = "TEtranscripts_DESeq2_normCounts_Emb_25C.txt", quote = FALSE,
...
volcanoTEtra + coord_cartesian(xlim = c(-1.5,1.5))
